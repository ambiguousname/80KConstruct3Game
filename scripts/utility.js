...
class Destroyer extends Enemy {
	...
	updatePath(self) { if (!self.dying && !self.dead && !self.immobile){var newX = (Math.floor(Math.random() * self.instance.runtime.layout.width)) - self.instance.x; var newY = (Math.floor(Math.random() * self.instance.runtime.layout.height)) - self.instance.y; console.log(newX); self.instance.behaviors.MoveTo.moveToPosition(self.instance.x + Math.min(Math.max(newX, -50), 50), self.instance.y + Math.min(Math.max(newY, -50), 50))}}
